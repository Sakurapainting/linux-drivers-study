...
static int myirqdev_fasync(int fd, struct file* file, int mode)
{
    struct myirq_dev* dev = (struct myirq_dev*)file->private_data;

    return fasync_helper(fd, file, mode, &dev->fasync);
}
