...
static unsigned int myirqdev_poll(struct file* file, struct poll_table_struct* wait)
{
    int mask = 0;
    struct myirq_dev* dev = (struct myirq_dev*)file->private_data;

    poll_wait(file, &dev->r_wait, wait); // 内核实现轮询 不是线程忙等
