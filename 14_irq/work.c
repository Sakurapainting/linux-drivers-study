...
{
    struct irq_keydesc* keydesc = (struct irq_keydesc*)data;
    // 这里可以通过 keydesc->gpio、keydesc->value 等区分是哪一个按键
    struct myirq_dev* dev = container_of(keydesc, struct myirq_dev, irqkey[0]);
...
{
    // myirq_dev -> irqkey[0] -> work
    struct irq_keydesc* keydesc = container_of(work, struct irq_keydesc, work);
    // 这里可以通过 keydesc->gpio、keydesc->value 等区分是哪一个按键
    struct myirq_dev* dev = container_of(keydesc, struct myirq_dev, irqkey[0]);
...
static int keyio_init(struct myirq_dev* dev)
{
    ...
    for (i = 0; i < KEY_NUM; i++) {
        ...
        INIT_WORK(&dev->irqkey[i].work, key_work); // 初始化工作队列
        // tasklet_init(&dev->irqkey[i].tasklet, key_tasklet, (unsigned long)&dev->irqkey[i]);
    }
